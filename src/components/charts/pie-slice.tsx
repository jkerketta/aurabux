...
function AnimatedSliceGrow({
  ...
  const growSpring = useSpring(outerRadius, { stiffness: 400, damping: 25 });
  ...
  const animatedPath = useTransform(
    [mountProgress, growSpring] as const,
  ...
  if (enterComplete) {
    return (
      <motion.path
        animate={{ opacity: isFaded ? 0.4 : 1, d: grownPath }}
        d={grownPath}
        fill={fill}
        pointerEvents="none"
        style={{
...
export const PieSlice = memo(function PieSlice({
  ...
  const renderStaticSlice = () => {
    ...
    return (
      <motion.path
        animate={{ opacity: isFaded ? 0.4 : 1, x: translateX, y: translateY }}
        d={hitboxPath}
        fill={fill}
        pointerEvents="none"
        style={{
